...
> **Currently parked:** `sooks-saga-scroll-07262026-6.html` — Build
> `07262026.6` (2026-07-26). Retain set is **3 files**: the 2 most recent
> overall (`-07262026-6` + `-07262026-5`, both 07-26) PLUS the most-recent
> previous-day build (`-07252026-4`, 07-25) as the cross-day rollback anchor.
> Pruned this park: `sooks-saga-scroll-07262026-4.html` → `_to_delete/`
> (untracked; git records the tracked path as deleted). **Roster "Solo" style
> match (Build 07262026.6, CSS-only, still v14, no schema change):** the guild
> roster's **"Solo"** status now uses the **same font/style as "Grouped" /
> "Guild Grouped"** — solid neutral ink (`--ink`) at `font-weight:600`, no italic
> (was faded `--ink-faded` + italic). Colour still distinguishes the state
> (neutral vs wax-red / emerald); only the typographic weight/style is unified.
> Verified in-browser (computed style: all three at weight 600 / normal). **Prior
...
## Files
- `sooks-saga-scroll-07262026-6.html` — the live build. Open in any
...
> currently `Build 07262026.6`. **Live-data note (important for future content
