...
> **Currently parked:** `sooks-saga-scroll-07302026-2.html` — Build
> `07302026.2` (2026-07-30). Retain set is **3 files**: the 2 most recent
> overall (`-07302026-2` + `-07302026-1`, both 07-30) PLUS the most-recent
> previous-day build (`-07292026-8`, 07-29) as the cross-day rollback anchor.
> Pruned this park: none (only 3 build files exist, all retained).
> **Build .2 — the Raids tab drops the keying/completion charline so its section
> headers line up with the Saga tab's.** Build .1 removed the redundant "⚔ Raids"
> title but left a "Keying & completion for &lt;char&gt;" charline above the first
> tier band, which pushed the Raids tab's "Heroic Raids" header lower than the Saga
> tab's "Heroic" header. Removed the charline AND its `.patrons-wrap` view wrapper
> entirely, so `renderRaidsView` now appends its tier-section bands straight to
> `#raidsView` exactly as the Saga tab appends to `#sagaList` — both first headers
> now sit at the same offset below the tab bar, governed by the shared 40px
> `.tier-section` top margin (the wrapper had only approximated it with a 30px
> `padding-top`). `.patrons-wrap` CSS stays (the Patrons tab still uses it). No
> schema change (still v15). Verified in-browser: Saga "Heroic" and Raids "Heroic
> Raids" headers render at the same vertical position.
> **Build .1 — the Raids tab drops its redundant "⚔ Raids" section header.** The
> Raids tab opened with a big "⚔ Raids" title directly under the tab bar,
> duplicating what the per-band tier headers ("Heroic Raids", "Epic Raids", …)
> already announce. Removed it so the tab now goes tab bar → keying charline →
> tier bands, mirroring the Saga tab (which has no "Saga" title — just its
> Heroic/Epic/Legendary bands). The character/keying charline is kept; it's an
> info line, not the section header. Markup-only removal in `renderRaidsView`;
> the now-unused `.raids-header` CSS rule is left in place. No schema change
> (still v15). Verified in-browser: Raids tab renders charline → "Heroic Raids"
> band with no redundant title.
...
> **title on its own full-width row** (`grid-template-areas: "t1 t1" / "t2 t3"`,
> was `"t1 t3" / "t2 t3"`) so long names no longer truncate — the pill sits
> **only on row two**, beside the progress bars, in a **narrower 150px band** (was
> 248px). (2) The pill is **shrunk** (width→150px, `min-height:62px→0`, padding
> `8/12→5/9`, and every row's font reduced). (3) **Row order reordered** to (1)
> **quest title** — a **dotted-underline DDO Wiki hyperlink** via the existing
> `_wikiHref()`, `target=_blank rel=noopener`, ellipsized with a full-name
> tooltip — (2) **leader with ★**, (3) **☠ reaper level + group-size fraction**
> (no longer italic). (4) A click on the quest link opens the wiki and **does not
> toggle the card** (the `.saga-header` handler bails on `e.target.closest("a")`).
> Verified in-browser: self-check 0 errors / 64 / 541 / 100%, 0 console errors;
> title spans full width while the pill sits below it on row two; row order
> quest→leader→reaper with r1 0.78rem / r2·r3·☠·★ 0.72rem; link href
> `ddowiki.com/page/The_Pit_of_Whispering_Shadows`, dotted underline; link-click
> = 0 card toggles, header-body click = 1. **Prior build — Group-status icons,
> unified reaper red, static Focus frame, LFM polish (Build 07262026.7, CSS + JS,
> still v14):** (1) each guild-roster group status gets a
> leading icon — **👤 Solo / 👥 Grouped / 🛡 Guild Grouped** (decorative,
> `aria-hidden`; the coloured label still carries meaning). (2) The **completed
> (sealed) reaper bar** now uses the **same solid `--wax-red`** as the in-progress
> bar (was a light→wax-red→dark gradient), keeping its `box-shadow` done-glow as
> the completion cue. (3) **Focus View emphasis is now a static illuminated gold
> "picture frame"** (inset 2px gold band + wax-red keylines + soft inner glow on
> `.saga-now-playing::before`) instead of the .5 opacity pulse — **zero
> animation**, lowest possible CPU; the `sagaNowPulse` keyframes are gone. (4)
> Clearer tooltips on the two Filters & Alerts toggles (Focus View → "focus
> content based on your location"; Switch prompt → "warns when you're in-game with
> a character that doesn't match the focus of your Sook's Saga Scroll"). (5) In
> the saga-header **LFM indicator**, the row-1 reaper skull ☠ is sized to match
> the row-2 leader ★ (both 0.84rem in a shared 1.1em leading slot), so ☠/★/quest
> share one left column and the three rows' first glyphs align; the quest name
> (row 3) is shrunk to 0.72rem. Verified in-browser: self-check 0 errors / 64 /
> 541 / 100%, 0 console errors; skull==star (13.44px, first glyphs + row edges all
> at the same x), sealed fill computed `rgb(138,31,26)` (`--wax-red`) with
> `backgroundImage:none` + done-glow, Focus frame static (no running animation)
> and unclipped by `overflow:hidden`. **Prior build — Roster "Solo" style match
> (Build 07262026.6, CSS-only, still v14):** the guild roster's **"Solo"** status
> uses the **same font/style as "Grouped" / "Guild Grouped"** — solid neutral ink
> (`--ink`) at `font-weight:600`, no italic. **Prior
> build — Focus View + Switch prompt toggles (Build 07262026.5, still v14, no
> schema change):** two toggles
> added to Filters & Alerts (a full-width "View & Alerts" `.view-group` under the
> 2×2, styled like the other items, each with a `title=` explanation). (1)
> **Focus View** (new, app-meta `focusView`, default ON) is the **master switch**
> for the location feature: ON → the "📍 you're in …" banner + `.saga-now-playing`
> highlight + a **pulse** on the sagas matching your active character's current
> area; OFF → none of it. Gated at render time via `_focusViewEnabled()` (banner
> condition + the highlight class), so toggling just re-renders the already-known
> `_nowPlaying`. The pulse was converted from an animated `box-shadow` (CPU-costly,
> no reduced-motion gate) to a **GPU-friendly opacity pulse** on an inset
> `.saga-now-playing::before` overlay (inset so `overflow:hidden` on banked cards
> can't clip it; `::before` is free — banked uses `::after`), gated by
> `prefers-reduced-motion`. (2) **Switch prompt** surfaces the existing
> `connectSwitchPrompt` setting (default ON) as a checkbox via
> `_setConnectSwitchEnabled` — the deferred "settings checkbox" finally lands.
> Both settings persist in app-meta and are wired by `bindViewToggles()`.
> Verified in-browser: self-check 0 errors / 64 / 541 / 100%, 0 console errors;
> both toggles render + default ON + persist; Focus View gates banner/highlight
> live (on→shown, off→hidden, back-on→shown); pulse is GPU-friendly + reduced-
> motion-gated. Plan:
> `docs/plans/2026-07-26-003-feat-focus-view-and-connect-switch-toggles-plan.md`.
> **Prior build — Real group status + completed-look polish (Build 07262026.4,
> still v14, no schema change):** (1)
> **Guild-roster group status is now REAL and three-state** — the roster reads
> `is_in_party` + `group_id` straight off each `online_characters` record (which
> sees private/full groups the LFM feed couldn't), replacing the best-effort
> "Pugging/blank". Row 4: **Solo** (not in a party — now honest), **Grouped** (in
> a party, no online guildmate in it), **Guild Grouped** (shares a `group_id`
> with ≥1 other online guildmate; tooltip names them). The old LFM-derived
> `_groupIndex` (and its `refreshLfmIndex` capture + reset sites) is **removed** —
> a net simplification. (2) **Banked-saga colour** now uses the exact quest-row
> zebra dark tone `rgba(101,78,40,0.26)` (flat), and the ribbon is an **elegant
> diagonal corner banner** (gold-trimmed emerald, `rotate(45deg)`, clipped to the
> corner by `overflow:hidden`) replacing the hanging pennant. (3) **LFM leader ★**
> gets a **white outline** so it reads on the orange/red LFM blocks. (4) **"Public
> Zone"** removed from roster row 3 (kept the story name + "Wilderness Zone").
> Verified in-browser: self-check 0 errors / 64 / 541 / 100%, 0 console errors;
> synthetic roster drove Solo / Grouped / Guild Grouped (+ who-with tooltip),
> diagonal ribbon + zebra colour, no "Public Zone". Plan:
> `docs/plans/2026-07-26-002-feat-real-group-status-and-completed-polish-plan.md`.
> **Prior build — Completed-look polish (Build 07262026.3, CSS-only, still v14,
> no schema change):** (1) **Completed
> progress bars are now FLAT** — the animated/static "shimmer" sheen band on
> `.all-done` / `.reaper.sealed` bars is removed (user preference), dropping the
> last GPU-composited animation on the page; the fill keeps its solid gradient.
> (2) **A banked-reward saga reads as COMPLETED** — when a container's reward is
> banked (the JS `.banked` state, or `.complete` when also all-quests-done), the
> whole card washes to the darker "zebra" tone (`rgba(101,78,40,…)`, the same
> dark stripe used on quest rows) and a small emerald **"✦ BANKED" ribbon** hangs
> off the top-right corner (CSS `::after`, notched tail via `clip-path`; anchored
> to the already-`position:relative` `.saga`). (3) **The saga-header LFM pill is
> left-justified** (was centred in its right band). Verified in-browser:
> self-check 0 errors / 64 / 541 / 100%, 0 console errors; flat bar (`::after`
> content `none`), banked dark-zebra bg + ribbon on both `.banked` and
> `.complete`, ribbon reads "✦ BANKED". **Prior build — Guild roster & LFM
> refinements (Build 07262026.2, still v14; adds two fields to the reference
> cache → `DDO_REF_EPOCH` 2→3, which forces a one-time refetch):**
> (1) **Roster is now 4 rows** — row 1 name (left) · abbreviated class+level
> (**right-justified**, reverting 07262026.1's all-left row 1); row 2 location
> (⚔ quest / 📍 zone, unchanged); **row 3 (new)** the **story/saga** the quest
> belongs to (via `_sagaForQuest`) or **"Wilderness Zone" / "Public Zone"**
> (from the areas table's new `is_wilderness`/`is_public` flags) or blank; **row 4**
> the group status. (2) **Group status is now "Pugging" / blank** (was
> Grouped/Solo): "Pugging" when in an advertised LFM group, **blank** otherwise —
> never "Solo", since the LFM feed can't see private/full groups, so absence
> isn't evidence of solo. Tooltip gives the group size + leader (★). (3) **Guild
> header** drops the `⟨ ⟩` brackets. (4) **Gold ★ marks the group leader**
> (`_leadStar`) across every LFM indicator — reaper/raid cards, saga-header tags,
> quest live line. (5) **Saga-header LFM tag** shows the **quest name** instead of
> the adventure zone/area (quest name attached to each group record). **Data
> note:** wilderness/public classification comes from `is_public`/`is_wilderness`
> on the DDO Audit `/areas` record (verified live); story = the app's own saga
> container (no adventure-pack fallback — the "pack" is a purchase bundle, not a
> story, so blank is more honest). A reliable per-character `group_id` probe (for
> true grouped/private/solo incl. private groups) stays deferred. Verified
> in-browser: self-check 0 errors / 64 / 541 / 100%, 0 console errors, epoch 3,
> synthetic roster driving all 4 rows across in-quest/wilderness/public/unknown +
> Pugging/blank, saga tag star + quest name. **Prior build (07262026.1) — Roster
> & progress-bar visual pass (still v14, no schema change):** four UI changes.
> (1) **Reaper bars
> all-red** — a fully sealed (100%-reaped) header reaper bar no longer shifts
> red→gold; the gold gradient/shimmer/border/glow are replaced by a deeper
> all-red gradient + red glow, and the partial reaper bar's inherited gold
> `.has-progress` border is overridden red too (empty bars stay quiet parchment;
> per-quest ☠ sealed *badges* are unchanged — out of scope). (2) **Matched,
> shrinking upper corners** — the upper-right character pills
> (`#charCorner .char-chip`, rendered by `renderCharCorner`) were a fixed 180px
> that overflowed the Characters card once `--corner-w` shrank to its 150px
> floor; now `width:100% / max-width:180px` so they shrink WITH the card and
> **clip** text (no ellipsis), keeping the upper-right cluster the same size as
> the upper-left guild card. Nothing is hidden; the ≤1040px narrow-stack is
> untouched. (3) **Guild roster redesign** — line 1 name + abbreviated classes
> (`Fighter/Wizard`→`Ftr/Wiz` via new `_abbrevClasses`), left-justified; line 2
> location left-justified (indent dropped); new line 3 **Grouped/Solo** with a
> hover tooltip naming the party, its slot reserved (min-height) so rows stay
> uniform. (4) **Dramatic banked pill** — the BANKED reward pill is now an
> embossed emerald wax-seal (glossy top highlight, deep gradient, green glow,
> cream text) kept to its exact 116×22px footprint, so it still matches
> INCOMPLETE and the two bars. **Grouped/Solo is BEST-EFFORT:** built from the
> advertised-LFM feed (`refreshLfmIndex`'s existing `/lfms` fetch — zero added
> network) into a new `_groupIndex` keyed by member name; a player in an
> unadvertised/full/private group reads "Solo" (false negative) and anonymous
> members can't be name-matched. A reliable per-character `group_id` live-probe
> was **deferred** (candidate future upgrade). Verified in-browser (localhost +
> Claude-in-Chrome): self-check **0 errors / 64 containers / 541 quests / 100%
> detailed**, 0 console errors; all four features driven — reaper partial+sealed
> red, banked pill, upper-right pills shrink+clip at the 150px floor (132px chip
> in a 150px panel, `text-overflow:clip`), and a synthetic roster showing
> abbreviations + left-justify + Grouped/Solo + who-with tooltips. **Prior build
> (07252026.4) — Connect-switch banner visibility (CSS-only, no schema/behavior
> change):** the 07252026.2
...
> `quest_id`/`area_id` was absent and **three live features silently failed for the
> new content** — (1) the guild roster's location line rendered blank
> (`questByArea`/`areasById` both miss the U81 `location_id`), (2) U81 **LFM badges
> never appeared** (`refreshLfmIndex` drops any LFM whose `quest_id` it can't
> resolve to a name, line ~14052), and (3) quest **Autocomplete never fired**
> (`location_id` → `getQuestAreaMap()` returned nothing → no quest name → no
> `_sagaForQuest`). Root cause: the row **schema** was unchanged, so the existing
> shape guards never forced a refetch — only the row **contents** changed. Fix: a
> reference-cache **epoch** (`DDO_REF_EPOCH`, currently `2`) stamped on every
> `_writeRefCache` and required by both read guards; any cache written under an
> older epoch (or with none) refetches **once**, so U81 IDs land immediately
> instead of after the ≤7-day TTL lapses. **Bump the epoch on each future content
> drop.** Verified in-browser (localhost + Claude-in-Chrome): with a stale
> epoch-less cache the old code would have trusted, the guard forced a refetch that
> pulled all 668 quests incl. U81 (`Back to the Abyss` area → quest resolves); a
> cache already at epoch 2 is trusted without a refetch; self-check **0 errors, 64
> containers / 541 quests / 100% detailed**. **Data note (for the U81 re-source):**
> DDO Audit lists a **13th** quest in the "Terror of Demogorgon" pack —
> **"The Darklake"** — that the container omits; likely a wilderness zone, verify
> on wiki. **Still-open preview-data gaps** (`TODO(u81-resource)`): 4 placeholder
> synopses, 2 placeholder locations, the Grin Ousttyl/Ousstyl giver spelling, full
> monster enumeration. Update this line on every park.
...
## Files
- `sooks-saga-scroll-07302026-2.html` — the live build. Open in any
  browser; no server or build step. Progress auto-saves to browser
  storage (key `sooksSagaScroll`, **schema v15** as of the Raids-tab
  build — see notes below). See **Data Schema & Import/Export
  ...
  **Build 07262026.8 — Slimmer saga-header LFM pill: off row one,
  ...
  The single live-LFM pill in a saga header was too bulky and squeezed long
  saga titles. (1) The header grid now gives the **title a full-width row**
  (`grid-template-areas: "t1 t1" / "t2 t3"`, was `"t1 t3" / "t2 t3"`) so long
  names no longer truncate; the pill sits **only on row two** beside the bars in
  a **narrower 150px band** (was 248px). (2) The pill is **shrunk** (width→150px,
  `min-height` 62→0, padding 8/12→5/9, all row fonts reduced). (3) **Rows
  reordered** to (1) **quest title** as a **dotted-underline DDO Wiki link**
  (`_wikiHref`, `target=_blank rel=noopener`, ellipsized + full-name tooltip),
  (2) **★ leader**, (3) **☠ reaper level + fraction** (no longer italic). (4)
  Clicking the quest link opens the wiki and **doesn't toggle the card** — the
  `.saga-header` click handler bails on `e.target.closest("a")`. Verified
  in-browser: self-check 0/64/541/100%, 0 console errors; title full-width with
  the pill below it on row two; order quest→leader→reaper (r1 0.78rem, rest
  0.72rem); link href correct + dotted underline; link-click = 0 toggles,
  header-body = 1.

  **Build 07262026.7 — Group-status icons, unified reaper red, static
  Focus frame, LFM polish (CSS + JS, no schema change, still v14).** Six
  refinements: (1) each guild-roster group status gains a leading icon —
  **👤 Solo / 👥 Grouped / 🛡 Guild Grouped** (`.gm-ico`, decorative /
  `aria-hidden`; the coloured label still carries meaning). (2) The
  **completed (sealed) reaper bar** drops its light→dark gradient for the
  **same solid `--wax-red`** as the in-progress bar, keeping its
  `box-shadow` done-glow as the completion cue (user preferred the
  in-progress red). (3) **Focus View emphasis is a static illuminated gold
  "picture frame"** (inset 2px gold band + wax-red keylines + inner glow on
  `.saga-now-playing::before`) replacing the .5 opacity pulse — **zero
  animation**, lowest CPU; `sagaNowPulse` keyframes removed. (4) Clearer
  `title=` tooltips on the Focus View / Switch prompt toggles. (5) In the
  saga-header **LFM indicator**, the row-1 skull ☠ is sized to match the
  row-2 leader ★ (both 0.84rem in a shared 1.1em slot → ☠/★/quest share one
  left column, first glyphs align); the quest name (row 3) shrinks to
  0.72rem. Verified in-browser: self-check 0/64/541/100%, 0 console errors;
  skull==star (13.44px, aligned at one x), sealed fill computed
  `rgb(138,31,26)` + done-glow, Focus frame static & unclipped.

  **Build 07262026.6 — Roster "Solo" style match (CSS-only, no schema
  change, still v14).** The guild roster's **"Solo"** status now uses the same
  font/style as "Grouped" / "Guild Grouped" — solid neutral ink (`--ink`) at
  `font-weight:600`, no italic (was faded ink + italic). Colour still marks the
  state; only weight/style is unified. Verified in-browser (all three at weight
  600 / normal).

  **Build 07262026.5 — Focus View + Switch prompt toggles (CSS + JS, no
  schema change, still v14).** Two toggles added to Filters & Alerts (a
  full-width "View & Alerts" group, styled like the other items). (1) **Focus
  View** (new, app-meta `focusView`, default ON) master-switches the location
  feature: ON → the "📍 you're in …" banner + `.saga-now-playing` highlight + a
  **pulse** on sagas matching your active character's area; OFF → none.
  Render-time gated via `_focusViewEnabled()`. The pulse moved from an animated
  `box-shadow` to a GPU-friendly opacity pulse on an inset `.saga-now-playing::before`
  overlay, gated by `prefers-reduced-motion`. (2) **Switch prompt** surfaces the
  existing `connectSwitchPrompt` on/off (default ON) as a checkbox — the deferred
  settings checkbox lands. Both persist in app-meta (`bindViewToggles`). Verified
  in-browser: self-check 0 errors / 64 / 541 / 100%, 0 console errors; toggles
  render + default ON + persist; Focus View gates banner/highlight live. Plan:
  `docs/plans/2026-07-26-003-feat-focus-view-and-connect-switch-toggles-plan.md`.

  **Build 07262026.4 — Real group status + completed-look polish (CSS + JS,
  no schema change, still v14).** (1) Guild-roster **group status is now real
  and three-state**, read from `is_in_party` + `group_id` on the roster's own
  `online_characters` records (sees private/full groups): **Solo** (not in a
  party), **Grouped** (in a party, no online guildmate in it), **Guild Grouped**
  (shares a `group_id` with ≥1 other online guildmate; tooltip names them). The
  old LFM-derived `_groupIndex` and "Pugging/blank" are **removed** (net
  simplification — direct fields are reliable and see private groups). (2)
  Banked-saga colour → exact quest zebra dark `rgba(101,78,40,0.26)`; ribbon →
  **diagonal gold-trimmed corner banner** (rotate 45°, clipped by
  `overflow:hidden`) replacing the pennant. (3) LFM leader **★** gets a white
  outline for the orange/red blocks. (4) **"Public Zone"** dropped from row 3.
  Verified in-browser: self-check 0 errors / 64 / 541 / 100%, 0 console errors;
  synthetic roster drove all three group states + who-with tooltip. Plan:
  `docs/plans/2026-07-26-002-feat-real-group-status-and-completed-polish-plan.md`.

  **Build 07262026.3 — Completed-look polish (CSS-only, no schema change,
  still v14).** (1) Completed progress bars are **flat** — the shimmer sheen
  band on `.all-done` / `.reaper.sealed` bars + the `progressSheen` animation
  are removed (the fill keeps its solid gradient). (2) A **banked-reward saga
  reads as completed**: the container washes to the darker zebra tone and a
  small emerald **"✦ BANKED" ribbon** hangs off the top-right corner (CSS
  `::after` + `clip-path` tail; applies to both `.banked` and the all-done
  `.complete`). (3) The **saga-header LFM pill is left-justified** (was
  centred). Verified in-browser: self-check 0 errors / 64 / 541 / 100%, 0
  console errors. Plan: none (direct build request).

  **Build 07262026.2 — Guild roster & LFM refinements (CSS + JS, no schema
  change, still v14; reference cache gains `is_public`/`is_wilderness` →
  `DDO_REF_EPOCH` 2→3).** (1) Roster is **4 rows**: row 1 name (left) +
  abbreviated class/level **right-justified** (reverts 07262026.1 all-left);
  row 2 location (unchanged); **row 3 (new)** story/saga (`_sagaForQuest`) or
  "Wilderness Zone" / "Public Zone" (area flags) or blank; row 4 group status.
  (2) Group status is **"Pugging" / blank** (was Grouped/Solo) — never "Solo",
  since the LFM feed can't see private/full groups; tooltip gives size + leader
  (★). (3) Guild header drops the `⟨ ⟩` brackets. (4) Gold **★** marks the group
  leader (`_leadStar`) across every LFM indicator. (5) Saga-header LFM tag shows
  the **quest name** instead of the adventure zone. Story = the app's saga
  container (no pack fallback — a pack is a purchase bundle, not a story).
  Verified in-browser: self-check 0 errors / 64 / 541 / 100%, 0 console errors,
  epoch 3, synthetic roster across all states. Plan:
  `docs/plans/2026-07-26-001-feat-guild-roster-lfm-refinements-plan.md`.

  **Build 07262026.1 — Roster & progress-bar visual pass (CSS + JS, no
  schema change, still v14).** Four UI changes. (1) **Reaper bars all-red:**
  a fully sealed header reaper bar no longer shifts red→gold — deeper all-red
  gradient + red glow, and the partial reaper bar's inherited gold
  `.has-progress` border is overridden red (per-quest sealed ☠ badges left
  as-is, out of scope). (2) **Matched, shrinking upper corners:** the
  upper-right character pills (`#charCorner .char-chip`) went from a fixed
  180px that overflowed the shrinking Characters card to `width:100% /
  max-width:180px`, so they narrow WITH the card to its 150px floor and clip
  (no ellipsis) — matching the upper-left guild card; nothing hidden, narrow
  stack untouched. (3) **Guild roster redesign:** line 1 name + abbreviated
  classes (`_abbrevClasses`, e.g. `Ftr/Wiz`), left-justified; line 2 location
  left-justified; new line 3 **Grouped/Solo** + who-with tooltip, slot
  reserved so rows stay uniform. (4) **Dramatic banked pill:** embossed
  emerald wax-seal styling at the unchanged 116×22px footprint. Grouped/Solo
  is best-effort (advertised-LFM feed → new `_groupIndex`; unadvertised/full
  groups read "Solo"; a reliable per-character `group_id` probe deferred).
  Verified in-browser: self-check 0 errors / 64 / 541 / 100%, 0 console
  errors, all four features driven.
  ...
  **Build 07242026.1 — U81 live-data fix: reference-cache epoch (no schema
  ...
  "Terror of Demogorgon" content: the guild-roster location line, U81 LFM badges,
  and quest Autocomplete. All three resolve against the DDO Audit `areas`/`quests`
  reference tables, cached 7 days in `localStorage` (key `sooksSagaScrollRef`,
  separate from `state`). When U81 dropped 2026-07-22, existing caches predated it,
  so every U81 `quest_id`/`area_id` was missing: `refreshLfmIndex` drops any LFM it
  can't name-resolve (line ~14052), and `getQuestAreaMap()` returned nothing for
  U81 `location_id`s — starving both the roster line and `_acApply`. The existing
  shape guards never forced a refetch because the row **schema** was unchanged; only
  the **contents** changed. Fix = a reference-cache **epoch**: new constant
  `DDO_REF_EPOCH` (currently `2`), stamped on every `_writeRefCache` and required by
  both the `getAreasMap`/`getQuestsMap` read guards (`cache.epoch === DDO_REF_EPOCH
  && …`). A cache written under an older/absent epoch is treated as stale and
  refetched once, so new content lands immediately instead of after the ≤7-day TTL.
  **Bump `DDO_REF_EPOCH` on each future expansion / content drop.** Verified
  in-browser (localhost + Claude-in-Chrome): a stale epoch-less cache the old code
  would have trusted forced a refetch to the full 668-quest live table incl. U81
  (`Back to the Abyss` area_id → quest name resolves); a cache already at epoch 2 is
  trusted with no refetch; self-check 0 errors, 64 containers / 541 quests / 100%
  detailed. Data note surfaced for the U81 re-source: DDO Audit lists a **13th**
  pack quest, **"The Darklake"**, not in the container (likely a wilderness zone —
  verify on wiki).
...
> L37, 12 quests each) as of Build 07222026.1; the live-data plumbing was fixed
> for that content in Build 07242026.1 (see below). Multi-character
> with
...
> `QUEST_DETAILS`). A cache written before new game content lacks its
> `quest_id`/`area_id`s, so those three features go blank for the new content until
...
> gets a full-width row (long names stop truncating); the pill is confined to row
> two beside the bars in a 150px band (was 248px). (2) Pill shrunk (width 150,
> `min-height` 0, padding 5/9, `.lfm-r1` 0.78rem / `.lfm-r2`·`.lfm-r3` 0.72rem).
> (3) Rows reordered to quest (a `_wikiHref` DDO Wiki link, dotted underline,
> `target=_blank`) / ★ leader / ☠ reaper·fraction (r3 no longer italic). (4) The
> `.saga-header` click handler bails on `e.target.closest("a")` so the quest link
> opens the wiki without toggling the card.**
> **Build 07262026.7 (still v14; CSS + JS, no schema change): group-status
> icons, unified reaper red, static Focus frame, LFM polish.** (1) Each
> guild-roster group status gets a leading `.gm-ico` — 👤 Solo / 👥 Grouped /
> 🛡 Guild Grouped (decorative, `aria-hidden`). (2) The sealed/completed reaper
> bar (`.reaper.sealed .saga-progress-fill`) drops its light→dark gradient for
> solid `var(--wax-red)` (same as the in-progress bar), keeping its done-glow
> `box-shadow`. (3) Focus View emphasis is a **static** illuminated gold frame on
> `.saga-now-playing::before` (inset gold band + wax-red keylines + inner glow) —
> the `.5` opacity pulse and its `sagaNowPulse` keyframes are gone (zero
> animation). (4) Clearer `title=` tooltips on the Focus View / Switch prompt
> toggles. (5) Saga-header LFM tag: the row-1 skull ☠ (new `.lfm-skull` span) is
> sized to match the row-2 leader ★ (both 0.84rem in a 1.1em leading slot, scoped
> to `.hdr-t3` so the global ★ is untouched), first glyphs align across rows 1-3,
> and the quest name (`.lfm-r3`) shrinks to 0.72rem.**
> **Build 07262026.6 (still v14; CSS-only, no schema change): roster "Solo"
> style match — the guild roster's `.gm-grp.solo` now uses the same font/style
> as `.grouped` / `.guild-grouped` (solid `--ink` at `font-weight:600`, not
> italic; was `--ink-faded` + italic). Colour still distinguishes the state;
> only weight/style is unified.**
> **Build 07262026.5 (still v14; CSS + JS, no schema change): Focus View +
> Switch prompt toggles.** Two toggles added to Filters & Alerts, tucked into a
> full-width "View & Alerts" `.view-group` under the 2×2, styled like Audio
> Alerts / Quest Autocomplete, each with a `title=` explanation. (1) **Focus
> View** (new; app-meta `focusView`, default ON) is the **master switch** for the
> location-emphasis feature: ON → the "📍 you're in …" banner + the
> `.saga-now-playing` highlight + a **pulse** on the sagas matching your active
> character's current area; OFF → none of them. Gated at render time via
> `_focusViewEnabled()` (the banner condition + the highlight class, checked last
> so it's only read for matched sagas), so toggling just re-renders the
> already-resolved `_nowPlaying` — no refetch. The pulse was converted from an
> animated `box-shadow` (CPU-costly, ungated) to a GPU-friendly **opacity** pulse
> on an inset `.saga-now-playing::before` overlay (inset so `overflow:hidden` on
> a banked card can't clip it; `::before` is free — banked uses `::after`), gated
> by `prefers-reduced-motion`. (2) **Switch prompt** surfaces the existing
> `connectSwitchPrompt` on/off (default ON) as a checkbox via
> `_setConnectSwitchEnabled` — the long-deferred settings checkbox. Both settings
> persist in app-meta, wired by `bindViewToggles()`; the setters also re-sync
> their checkbox. Verified in-browser (self-check 0 errors / 64 / 541 / 100%, 0
> console errors; toggles render + default ON + persist; Focus View gates
> banner/highlight live; pulse GPU-friendly + reduced-motion-gated). Plan:
> docs/plans/2026-07-26-003-feat-focus-view-and-connect-switch-toggles-plan.md.**
> **Build 07262026.4 (still v14; CSS + JS, no schema change): real group status
> + completed-look polish.** (1) The guild roster's **group status is now real
> and three-state**, read from `is_in_party` + `group_id` on each
> `online_characters` record (a live probe confirmed these fields ride the guild
> fetch — and they see private/full groups the LFM feed couldn't): **Solo** (not
> in a party — now honest), **Grouped** (in a party, no online guildmate in it),
> **Guild Grouped** (shares a nonzero `group_id` with ≥1 other online guildmate;
> tooltip names them, via a within-roster `group_id` tally). The old LFM-derived
> `_groupIndex` — its declaration, `refreshLfmIndex` capture, all reset sites,
> and the "Pugging/blank" labels — is **removed** (net simplification; the LFM
> feed still powers reaper badges/cards). (2) Banked-saga background → the exact
> quest-row zebra dark tone `rgba(101,78,40,0.26)` (flat); the ribbon is now an
> **elegant diagonal corner banner** (gold-trimmed emerald, `rotate(45deg)`,
> clipped to the corner by `overflow:hidden` on `.banked`/`.complete`) replacing
> the hanging pennant. (3) The LFM leader **★** (`.lfm-star`) gets a **white
> outline** so it reads on the orange (mid) / red (high) LFM blocks. (4) **"Public
> Zone"** removed from roster row 3 (kept the story name + "Wilderness Zone").
> Verified in-browser (self-check 0 errors / 64 / 541 / 100%, 0 console errors;
> synthetic roster drove Solo / Grouped / Guild Grouped with the who-with tooltip;
> diagonal ribbon + zebra colour; no "Public Zone"). Plan:
> docs/plans/2026-07-26-002-feat-real-group-status-and-completed-polish-plan.md.**
> **Build 07262026.3 (still v14; CSS-only, no schema change): completed-look
> polish.** (1) Completed progress bars are now **flat** — the shimmer sheen
> band on `.all-done` / `.reaper.sealed` bars and the `progressSheen` animation
> are removed (user preference); the fill keeps its solid gradient. (2) A
> **banked-reward saga reads as completed**: the container background washes to
> the darker "zebra" tone (`rgba(101,78,40,…)`) and a small emerald **"✦ BANKED"
> ribbon** hangs off the top-right corner (CSS `::after`, notched `clip-path`
> tail; anchored to the already-`position:relative` `.saga`). Applies to both
> the `.banked` state and the all-done `.complete` state (both mean reward
> banked). (3) The **saga-header LFM pill is left-justified** (`.hdr-t3`
> `justify-content: flex-start` + pill content `align-items: flex-start` /
> `text-align: left`; was centred in its right band). Verified in-browser
> (self-check 0 errors / 64 / 541 / 100%, 0 console errors; flat-bar `::after`
> content `none`, banked dark-zebra + "✦ BANKED" ribbon).**
> **Build 07262026.2 (still v14; CSS + JS; reference cache gains
> `is_public`/`is_wilderness` → `DDO_REF_EPOCH` 2→3, one-time refetch): guild
> roster & LFM refinements.** (1) Roster is now **4 rows**: row 1 name (left) +
> abbreviated class/level **right-justified** (reverts 07262026.1 all-left row 1);
> row 2 location (⚔ quest / 📍 zone); **row 3 (new)** the story/saga the quest
> belongs to (`_sagaForQuest`) or "Wilderness Zone" / "Public Zone" (from the
> areas table's new `is_wilderness`/`is_public`) or blank; row 4 group status.
> (2) Group status is **"Pugging" / blank** (was Grouped/Solo) — never "Solo",
> because the LFM feed can't see private/full groups; tooltip gives group size +
> leader (★). (3) Guild header drops the `⟨ ⟩` brackets. (4) Gold **★** marks the
> group leader (`_leadStar`) across every LFM indicator (reaper/raid cards,
> saga-header tags, quest live line). (5) Saga-header LFM tag shows the **quest
> name** not the adventure zone (quest name attached to each group record). Story
> uses the app's own saga container (no adventure-pack fallback — a pack is a
> purchase bundle, not a story). Reliable per-character `group_id` still deferred.
> Verified in-browser (self-check 0 errors / 64 / 541 / 100%, 0 console errors,
> epoch 3, synthetic roster across in-quest/wilderness/public/unknown +
> Pugging/blank, saga tag star + quest name). Plan:
> docs/plans/2026-07-26-001-feat-guild-roster-lfm-refinements-plan.md.**
> **Build 07262026.1 (still v14; CSS + JS, no schema change): roster &
> progress-bar visual pass — four UI changes. (1) Header REAPER bars are now
> ALL-RED in every state: the fully-sealed bar's red→gold gradient/shimmer/
> border/glow are gone (deeper all-red gradient + red glow instead), and the
> partial reaper bar's inherited gold `.has-progress` border is overridden red
> (per-quest sealed ☠ badges unchanged — out of scope). (2) Upper-right
> character pills (`#charCorner .char-chip`, from `renderCharCorner`) shrink WITH
> the Characters card (`width:100% / max-width:180px`, was a fixed 180px that
> overflowed the 150px `--corner-w` floor) and clip (no ellipsis), matching the
> upper-left guild card; nothing hidden, ≤1040px narrow-stack untouched. (3)
> Guild roster: line 1 name + abbreviated classes (`_abbrevClasses`, `Ftr/Wiz`),
> left-justified; line 2 location left-justified; new line 3 Grouped/Solo +
> who-with tooltip, slot reserved for uniform rows. (4) BANKED reward pill
> restyled dramatic (embossed emerald wax-seal) at its unchanged 116×22px
> footprint. Grouped/Solo is BEST-EFFORT: from the advertised-LFM feed
> (`refreshLfmIndex`'s `/lfms` fetch → new `_groupIndex` by member name; zero
> added network); unadvertised/full groups read "Solo", anon members can't be
> name-matched; a reliable per-character `group_id` live-probe is DEFERRED.
> Verified in-browser (self-check 0 errors / 64 / 541 / 100%, 0 console errors,
> all four features driven incl. a synthetic roster).**
...
> LFM / Autocomplete failures caused by pre-U81 caches surviving their 7-day TTL.
> Also surfaced for the U81 re-source: DDO Audit lists a 13th "Terror of
> Demogorgon" pack quest, "The Darklake", the container omits (likely a wilderness
> zone).**
> **Build 07222026.1 (still v14; data-only): DDO
